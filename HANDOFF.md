...
_Last updated: 2026-09-14 — methodology/design complete; Manifest v1.0 frozen executable; authoritative v0.1 contracts + Manifest + PRDs on `main` (PR #1). Physical schema split into ordered migrations `001`–`022` + Manifest v1.0/QA seeds, validated on local pgvector (1,100 → 1,000/91/9), merged. Stage-1 vertical-slice **Maps + Organic** collector built + validated offline (PR #10). Pilot finding: Maps zoom `17z` returned no results; locked to `14z` by owner-approved amendment (ADR-0006, migration `022`). Migrations `001`–`022` applied to the persistent PRODUCTION Supabase project (`wbqcvqxmhqyspgqsdpsm`), reconciling 1,100 → 1,000/91/9. **The first paid single-coordinate spike is now DONE and verified on PRODUCTION for BOTH surfaces (step 7 complete): Maps 10/10 businesses resolved by place_id, Organic 14 result rows → 9 web destinations resolved URL-first (0 business_locations), $0.0006 each. The bounded 3×5 Maps+Organic pilot (step 8) was EXECUTED on production and evaluated **COMPLETE** under QA/Wave-Acceptance v0.1 (step 9): wave `PILOT-3x5-20260914`, 1,368/1,368 executable returned (100%) + 192 structurally excluded, all rates 1.0, all 30 strata pass, 0 identifier splits, ~$0.8142 total. Steps 1–9 done; next is Step 10 (Full Panel / Sentinel). AIO & ChatGPT remain behind their ADR-0005 capture probes.**_
...
Steps 1–9 are **done**; the Stage-1 pilot is **COMPLETE** and production-promotable. Remaining:

1. **Proceed to Step 10 — Full Panel / Sentinel operation** without silent methodology drift (monthly Full Panel + weekly fixed Sentinel; the monthly wave doubles as that week's Sentinel). Reuse the validated harness (`collector/pilot.py` primitives / `run_spike`) at panel scale; the paid pilot proved cost (~$0.0006/task), reliability (transient provider errors are recoverable via resume), throughput (parallel workers; DataForSEO standard async queue ≈ 30 s/task, so run with `--workers`), and data quality/entity-safety on production.
2. **AIO & ChatGPT stay behind their own ADR-0005 capture probes** (out of current scope). Enrichment / findings / Client Mode remain deferred.
3. **Optional harness follow-ups (non-blocking):** re-queue an errored worker's remaining specs (rather than record-and-continue) for even smoother large runs; consider sharding the web-entity advisory-lock key at Full-Panel scale.

**Outstanding owner cleanup (non-blocking):** delete the stray Railway service `zucchini-ambition`; rotate the production DB password AND the `service_role` key (both shared in chat historically) and re-update `SUPABASE_DB_URL` / `SUPABASE_SERVICE_ROLE_KEY` afterward.
