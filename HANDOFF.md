...
- Step 7 **done on the dev branch, pending on production**: the single-coordinate spike ran end-to-end live on the (now-deleted) `lsi-dev` branch. It has **not** yet been re-run against production (that is the next paid call).
- Steps 8–10 **remaining**.

## Immediate next action

Infra + secrets are ready; the schema is live on production. Remaining:

1. **Run the single-coordinate spike on production** (step 7, **first paid DataForSEO call**) — set `RUN_PAID_SPIKE=1` (optionally `SPIKE_SURFACE=maps|organic`) on the Railway service and redeploy: one `task_post` for one eligible pilot coordinate → immutable raw → parse → normalize → resolve → cost ledger. **Gated on explicit owner confirmation.** Recommended order: one Maps call, verify the rows, then one Organic call to validate that path live. Reset `RUN_PAID_SPIKE=0` afterward.
2. **Execute the bounded 3×5 pilot** (step 8) from the frozen v1.0 executable matrix, then evaluate under the QA contract (step 9).
