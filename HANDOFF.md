...
_Last updated: 2026-09-14 — methodology/design complete; Manifest v1.0 frozen executable; authoritative v0.1 contracts + Manifest + PRDs on `main` (PR #1). Physical schema split into ordered migrations `001`–`022` + Manifest v1.0/QA seeds, validated on local pgvector (1,100 → 1,000/91/9), merged. Stage-1 vertical-slice **Maps + Organic** collector built + validated offline (PR #10). Pilot finding: Maps zoom `17z` returned no results; locked to `14z` by owner-approved amendment (ADR-0006, migration `022`). Migrations `001`–`022` applied to the persistent PRODUCTION Supabase project (`wbqcvqxmhqyspgqsdpsm`), reconciling 1,100 → 1,000/91/9. **The first paid single-coordinate spike is now DONE and verified on PRODUCTION for BOTH surfaces (step 7 complete): Maps 10/10 businesses resolved by place_id, Organic 14 result rows → 9 web destinations resolved URL-first (0 business_locations), $0.0006 each. The bounded 3×5 Maps+Organic pilot harness (step 8) is now BUILT and offline-validated (`collector/pilot.py`: 1,560 → 1,368 executable / 192 water-excluded, deterministic idempotency, bounded retries, QA/Wave-Acceptance v0.1 evaluator); it stays gated on explicit owner go before any paid run.**_
...
- Step 8 **harness built + offline-validated (2026-09-14)**: `collector/pilot.py` (matrix iteration + water gate + bounded retries + QA/cost telemetry + wave evaluator) and `scripts/validate_pilot.py`; the **paid execution** of the pilot remains gated on explicit owner go. Steps 9–10 follow (step 9 = evaluate the executed wave under the QA contract; the evaluator is implemented and validated offline).

## Immediate next action

Infra + secrets are ready; the schema is live on production; the single-coordinate spike is **done and verified on production for both surfaces** (step 7); the **3×5 pilot batch run harness is built and offline-validated** (`collector/pilot.py`, gated). Remaining:

1. **Execute the bounded 3×5 pilot** (step 8) from the frozen v1.0 executable matrix — 15 cells (3 industries × 5 markets) × 4 queries × 13 points × 2 surfaces = 1,560 pre-water jobs → **1,368 executable** (57 eligible coordinates × 3 industries × 4 queries × 2 surfaces) + **192 structurally excluded** after the water gate. This is a real collection run (many paid DataForSEO calls, est. ≈ $0.82 at the spike unit price), so it stays **gated on explicit owner go**. The harness (`collector/pilot.py`) does matrix iteration over the executable matrix with deterministic idempotency, bounded retries, QA/cost telemetry, and a step-9 COMPLETE/PARTIAL/FAILED/QUARANTINED evaluator — run it via the Railway service with `RUN_PAID_PILOT=1` (a gate distinct from `RUN_PAID_SPIKE`), or `python -m collector.pilot --execute`.
