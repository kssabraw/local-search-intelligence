...
Steps 1–9 are **done** (Stage-1 pilot **COMPLETE** and production-promotable). **Step 10 — Full Panel / Sentinel operation for Maps + Organic — is in progress**, governed by **ADR-0007** + `docs/design/full-panel-sentinel-scheduler-v0_1.md` (no silent methodology drift). Progress:
...
4. **Build step 3 built + offline-validated + merged (PR #22):** the cadence driver (`collector/panel_driver.py`) + the `RUN_PAID_PANEL` gate + Railway wiring (`scripts/railway_run.sh`, `PANEL_*` vars). **Step 4 (live) EXECUTED:** the graduated first paid Sentinel wave `SENTINEL-2026W38` ran on production and evaluated **COMPLETE** (~$2.64); `RUN_PAID_PANEL` re-closed. **Remaining:** a **Full Panel** wave (118,000 jobs, ~$70.80) on explicit owner "go" — not before.
5. **AIO & ChatGPT stay behind their own ADR-0005 capture probes** (out of current scope). Enrichment / findings / Client Mode remain deferred.
6. **Optional harness follow-ups (non-blocking):** re-queue an errored worker's remaining specs; shard the web-entity advisory-lock key at Full-Panel scale; DataForSEO postback/pingback instead of `tasks_ready` polling.
...
### Panel scheduler (step 10) — built + offline-validated (steps 1–3); first paid Sentinel wave COMPLETE (step 4)
...
- **`collector/panel_run.py`** — `PanelRunner` (submit → collect → reconcile) on the Standard decoupled adapter (`dataforseo.task_post_batch` ≤100/POST + `tasks_ready`); idempotency at submission (one paid task per job, collect-only on resume, never re-POSTed); reuses the water gate, `spike.finalize_collected`, cost ledger, and `pilot.evaluate_wave`. `scripts/validate_panel_run.py` asserts a clean run (QA COMPLETE), an idempotent resume that re-POSTs nothing, and reconcile (never-ready → accounted collect_timeout → QA FAILED, not QUARANTINED) — no paid call. Providers mocked in tests.
...
- **Remaining (step 4 cont.):** a **Full Panel** wave (118,000 jobs, ~$70.80) on explicit owner "go" — Sentinel-first is satisfied; no Full Panel paid call before the owner's "go".
