...
4. **Build step 3 built + offline-validated:** the cadence driver (`collector/panel_driver.py`) + the `RUN_PAID_PANEL` gate + Railway wiring (`scripts/railway_run.sh`, `PANEL_*` vars). Providers mocked; no paid call. **Remaining (step 4, live):** a graduated **paid Sentinel wave on explicit owner "go" (Sentinel-first)** → evaluate COMPLETE → only then a Full Panel.
...
### Panel scheduler (step 10) — built + offline-validated (steps 1–2), gated
...
- **Remaining (step 4, live):** the graduated **paid Sentinel wave on explicit owner "go" (Sentinel-first)** → evaluate COMPLETE → only then a Full Panel. No paid panel call before the owner's "go".
