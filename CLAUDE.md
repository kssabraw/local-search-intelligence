...
- **Foundation + schema live.** Migrations `001`–`026` are applied and reconciled on the **production** Supabase project (now 1,750 coordinates → 1,595 eligible / 141 structural-water / 14 outside-country, after `026` adds `GEOGRID13E_V1`; the frozen `MAPORG13_V1`/`AIO9_V1` universe was 1,100 → 1,000/91/9). One owner-approved reconciliation: `manifest.coordinate_eligibility` gains `outside_country_exclusion` (see `supabase/migrations/README.md`). Amendments: `022` Maps zoom `17z→14z` (ADR-0006), `023` versioned DataForSEO price, `024` measured AI-Mode task price (now historical — AI Mode deferred), `025` AIO organic-overview schema + `DFS_AIO_V2` provider profile + repoint (ADR-0008), `026` **efficient unified geo-grid `GEOGRID13E_V1`** (ADR-0009: center + N/E/S/W @3/5 mi + NE/SE/SW/NW @4 mi; drops the 1-mile ring; maps/organic/aio all repointed to it; `MAPORG13_V1`/`AIO9_V1` retained as history). The Railway service is live with all paid gates (`RUN_PAID_SPIKE` / `RUN_PAID_PILOT` / `RUN_PAID_PANEL` / `RUN_AIO_PROBE`) **closed** (`0`).
