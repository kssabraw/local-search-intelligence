...
- **Foundation + schema live.** Migrations `001`–`025` are applied and reconciled on the **production** Supabase project (1,100 coordinates → 1,000 eligible / 91 structural-water / 9 outside-country). One owner-approved reconciliation: `manifest.coordinate_eligibility` gains `outside_country_exclusion` (see `supabase/migrations/README.md`). Amendments: `022` Maps zoom `17z→14z` (ADR-0006), `023` versioned DataForSEO price, `024` measured AI-Mode task price (now historical — AI Mode deferred), `025` AIO organic-overview schema + `DFS_AIO_V2` provider profile + repoint (ADR-0008). The Railway service is live with all paid gates (`RUN_PAID_SPIKE` / `RUN_PAID_PILOT` / `RUN_PAID_PANEL` / `RUN_AIO_PROBE`) **closed** (`0`).
